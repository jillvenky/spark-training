# Databricks notebook source exported at Tue, 11 Aug 2015 20:17:05 UTC
# MAGIC %md
...
rdd = sc.textFile("dbfs:/home/training/dataframes/people.txt")
...
# MAGIC 
# MAGIC - Can't create datetime or int (fails at type conversion)
# MAGIC - Unescaped colons in data (fails at split)
# MAGIC - some records -/+ 6 columns (fails to create Person)
...
# MAGIC -- show people born in the 70's ordered by age and salary 
...
# MAGIC -- show people with salaries < 50k
...
df.show(20) # show the first 20 rows of the DataFrame
...
#display(df.freqItems(["birth_date","salary"]))
...
# MAGIC 
# MAGIC subset of data of first name, last name, gender? NO returns DataFrame
...
# MAGIC Applies a transformation to the DataFrame which looks for M in gender "column"
...
#display(df2)
df2.registerTempTable("MaleSalaries")
sqlContext.sql("SELECT last_name, first_name, salary FROM MaleSalaries WHERE salary>50000 ORDER BY last_name").show()
...
display( df.groupBy(df["birth_date"], df["gender"]).mean("salary") )
...
x = df.groupBy(df["salary"]).count()  # What is x? What is its type? DataFrame, table of salary & count
