# Databricks notebook source exported at Wed, 12 Aug 2015 20:56:35 UTC
# MAGIC %md
# MAGIC version 1.0.3
...
print rawMovies.getNumPartitions()
...
# MAGIC 
# MAGIC takeOrdered(N, key=function): takes N elements from RDD in order that function sorts
...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    avgRating = sum(IDandRatingsTuple[1]) * 1.0 /numRatings
    return (id, (numRatings, avgRating))
...
# MAGIC 
# MAGIC The RMSE is the square root of the average value of the square of `(actual rating - predicted rating)` for all users and movies for which we have the actual rating. Versions of Spark MLlib beginning with Spark 1.4 include a [RegressionMetrics](https://spark.apache.org/docs/latest/api/python/pyspark.mllib.html#pyspark.mllib.evaluation.RegressionMetrics) module that can be used to compute the RMSE. 
# MAGIC 
# MAGIC Write a function that returns the RMSE given `predictedRDD` and `actualRDD` RDDs. Both RDDs consist of tuples of the form (UserID, MovieID, Rating)
# MAGIC 
# MAGIC Given two ratings RDDs, *x* and *y* of size *n*, we define RMSE as follows: \\[ RMSE = \sqrt{\frac{\sum_{i = 1}^{n} (x_i - y_i)^2}{n}} \\]
# MAGIC 
...
# MAGIC * Create an RDD that, for each *matching* entry (i.e., the same (UserID, MovieID) in each RDD) in the reformatted RDDs, has a tuple of (prediction, label) - do *not* use `collect()` to perform this step. Note that not every (UserID, MovieID) pair will appear in both RDDs - if a pair does not appear in both RDDs, then it does not contribute to the RMSE. 
# MAGIC * Use `RegressionMetrics` with the predictions and labels RDD.
# MAGIC * Return the RMSE from the calculated metrics.
...
def computeError(predictedRDD, actualRDD):
    ...
    Returns:
        RSME (float): computed RSME value using RegressionMetrics
    """
    ...
    # Find the corresponding prediction and label for each matching entry (i.e., the same (User ID, Movie ID) 
    # in each RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    # This should be an RDD of (prediction, label).  Make sure the values are floats by using float().
    predictionAndLabelRDD = (predictedReformattedRDD
                             <FILL IN>)
    
    # Use RegressionMetrics to calculate the errors
    regressionMetrics = <FILL IN>
...
def computeError(predictedRDD, actualRDD):
    ...
    predictionAndLabelRDD = (predictedReformattedRDD
                            ...
                            .map(lambda x: (float(x[1][0]), float(x[1][1]))))
    
    # Use RegressionMetrics to calculate the error
    regressionMetrics = RegressionMetrics(predictionAndLabelRDD)
...
print '(average rating, movie id, movie name, number of reviews)'
for ratingsTuple in (movieLimitedAndSortedByRatingRDD
                     .map(lambda (r, n, c): (n, (r, c)))
                     .join(moviesRDD.map(lambda (k, v): (v, k)))
                     .map(lambda (n, ((r, c), i)): (r, i, n, c))
                     .takeOrdered(70, lambda x: -x[0])):
    print u'{0:.3f},  {1: >4},  {2: <54.54},  {3:,}'.format(*ratingsTuple)

# COMMAND ----------

# MAGIC %md
# MAGIC 
# MAGIC The user ID 0 is unassigned, so we will use it for your ratings. We set the variable `myUserID` to 0 for you. Next, create a new RDD `myRatingsRDD` with your ratings for at least 10 movie ratings. Each entry should be formatted as `(myUserID, movieID, rating)` (i.e., each entry should be formatted in the same way as `trainingRDD`).  As in the original dataset, ratings should be between 1 and 5 (inclusive). If you have not seen at least 10 of these movies, you can increase the parameter passed to `takeOrdered()` in the above cell until there are 10 movies that you have seen (or you can also guess what your rating would be for movies you have not seen).
