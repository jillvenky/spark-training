# Databricks notebook source exported at Wed, 12 Aug 2015 21:02:30 UTC
# MAGIC %md
# MAGIC 
...
#Lab exercise:
x = sc.parallelize([1,2,3])
y = x.filter(lambda x:x%2 ==0) #add a lambda parameter to keep only even numbers
print(x.collect())
print(y.collect())
...
x = sc.parallelize(['John', 'Fred', 'Anna', 'James', 'Maggie'])
...
z = x.keyBy(lambda x: x.split(",")[1].strip()).map(lambda (x,y): (x, y.split(",")[0]))
print z.collect()
...
seqOp = lambda aList, n: aList + [n] #how to put initial value and x together?
combOp = lambda l1, l2 : l1+l2 #how to combine elements above?

y = x.aggregate([0], seqOp, combOp) #add correct parameters
